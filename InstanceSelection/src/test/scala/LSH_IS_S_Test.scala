...
class LSH_IS_S_Test extends FunSuite with BeforeAndAfterAll {
  ...
  test("Se selecciona una instancia de cada clase por cubeta con clases balanceadas"){
    ...
    val isSDf = instances
                ...
                .as("select_instances"))
    val isS =  isSDf.select("select_instances").collect
    assert(isS(0)(0).asInstanceOf[Seq[Int]].size == 3)
    assert((isS(0)(0).asInstanceOf[Seq[Int]](0) == 2))
    assert((isS(0)(0).asInstanceOf[Seq[Int]](1) == 6))
    assert((isS(0)(0).asInstanceOf[Seq[Int]](2) == 7))
